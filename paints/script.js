...
var checkTextInputs = function checkTextInputs(selector) {
  var textInpunts = document.querySelectorAll(selector);
  textInpunts.forEach(function (textInput) {
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener("drop", function (e) {
      ...
      input.previousElementSibling.textContent = name; // let image = input.closest(".file_upload");
...
var filter = function filter() {
  ...
  menu.addEventListener("click", function (e) {
    if (e.target && e.target.tagName == "LI") {
      ...
      markAll.forEach(function (element) {
      ...
      if (e.target.classList[0] == "grandmother" || e.target.classList[0] == "granddad") {
        ...
        document.querySelectorAll(".".concat(e.target.classList[0])).forEach(function (el) {
...
var modal = function modal() {
  ...
  function bindModals(triggerSelector, modalSelector, closeSelector) {
    ...
    var close = modal.querySelector(".".concat(closeSelector));
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      ...
      if (!ifAnyModalShow) {
        document.querySelector(".".concat(selector)).style.display = "block";
        document.body.style.overflow = "hidden";
        document.body.style.marginRight = "".concat(scroll, "px");
      }
    }, time);
  ...
  scrollToTheEnd(".fixed-gift"); //showModalByTime("popup-consultation", 5000);
};
...
var showMoreStyles = function showMoreStyles(trigger, cards) {
...
var showpicture = function showpicture() {
  var wrapper = document.querySelector(".sizes-wrapper");
  var item = wrapper.querySelectorAll(".sizes-block");
  item.forEach(function (picture) {
    picture.addEventListener("mouseenter", function (e) {
      var img = e.target.querySelector("img");
      img.src = img.src.slice(0, -4) + "-1.png";
      e.target.querySelectorAll("p:not(.sizes-hit)").forEach(function (e) {
  ...
  item.forEach(function (picture) {
    picture.addEventListener("mouseleave", function (e) {
      ...
      e.target.querySelectorAll("p").forEach(function (e) {
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  activateAnimation();
  allSlides[0].parentElement.addEventListener("mouseenter", function () {
    clearInterval(paused);
  });
  allSlides[0].parentElement.addEventListener("mouseleave", function () {
    activateAnimation();
  });
};
